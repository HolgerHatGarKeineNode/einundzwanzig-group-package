 ...
 *
 * **kind 1111 ist hier ersatzlos raus** (P4): Thread-Antworten sind bei Buzz kind-9-
 * Nachrichten mit `reply`-Marker (`threading.ts`) — sie fallen also schon unter `MESSAGE`
 * und überleben den Kaltstart ohne eigenen Eintrag. Der frühere Sonderweg (eigenes Kind,
 * eigener globaler Deckel) entfällt damit; Antworten werden wie jede andere Nachricht pro
 * RAUM gekappt ({@link messagesToPrune}). Preis: sie teilen sich das Raum-Budget mit den
 * Wurzeln — richtig so, sie liegen ja auch im selben Raum.
 *
 * §4.2 raus:
 * Ephemeral/AUTH/Reaktionen/Zaps (kein `#h`, laden lazy nach dem Paint).
 */
...
/**
 * §4.3 Pruning — kind 9 wächst unbegrenzt (Control-Plane ist replaceable → selbst-bounded,
 * kein Cap): per-Raum die neuesten N behalten, plus Alters-Backstop als harte Obergrenze
 * (fängt zugleich tombstone-lose Relay-Purges, §6). Kein LRU-Framework.
 *
 * **Der frühere zweite Deckel für kind 1111 ist entfallen** (P4): Thread-Antworten sind
 * kind-9-Nachrichten MIT `h` und werden damit vom Raum-Cap miterfasst. Das schließt sogar
 * eine alte Lücke — der globale Kommentar-Deckel konnte Antworten eines geschwätzigen
 * Threads gegen die eines anderen ausspielen; jetzt gilt für alles im Raum dieselbe Regel.
 */
 ...
 * neuesten `cap`, dazu alles älter als `maxAgeSec`. Control-Plane bleibt unangetastet.
...
function syncEvents(): () => void {
    return on(
        ...
        batch(3000, async (updates: RepositoryUpdate[]) => {
            ...
            // §4.3: nach neuen Nachrichten den (bounded) Store kappen. Der events-Store ist
            // durchs Cap selbst begrenzt → getAll bleibt günstig. Thread-Antworten sind
            // selbst kind 9 — ein reiner Antwort-Burst löst die Kappung also mit aus.
