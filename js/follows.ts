 ...
 *
 * **„Forward in time" is not „larger".** A newer event can carry fewer entries — that is
 * what an unfollow is, and it is also what the one-tag stub of every finding on this path
 * looks like. Measured: targets serving the real 700-entry list plus a hint serving a
 * newer, validly signed one-entry event yields a base of one, and that base is correct by
 * NIP-01. So a hint makes the base MORE current, never larger — which is the guarantee
 * this list carries and the only one it carries.
 *
 * What would make it laxer is letting it
